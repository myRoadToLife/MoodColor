...
namespace App.Editor
{
    public class AddressablesSetupTool : EditorWindow
    {
        ...
        private void InitializeAddressableAssetSettings()
        {
            ...
            {
                MyLogger.LogWarning("Группа по умолчанию не была автоматически создана или найдена. Создаю 'Default Local Group'.", MyLogger.LogCategory.Editor);
            ...
            MyLogger.Log("Система Addressables успешно инициализирована или существующие настройки загружены.", MyLogger.LogCategory.Editor);
        }
        ...
        private void CreateAddressableGroups()
        {
            if (!IsAddressablesInitialized())
            {
                MyLogger.LogError("Addressable Asset Settings не найдены! Сначала инициализируйте систему.", MyLogger.LogCategory.Editor);
                return;
            }
            ...
            foreach (var groupType in _groupTypes)
            {
                ...
                {
                    MyLogger.Log($"Группа '{groupType}' уже существует. Пропускаю создание.", MyLogger.LogCategory.Editor);
                ...
                MyLogger.Log($"Создана группа Addressables: '{groupType}'", MyLogger.LogCategory.Editor);
            }

            settings.SetDirty(AddressableAssetSettings.ModificationEvent.GroupAdded, null, true);
            AssetDatabase.SaveAssets();
            MyLogger.Log("Создание/обновление групп Addressables завершено.", MyLogger.LogCategory.Editor);
        }
        ...
        private void AssignAddressesToAssets()
        {
            ...
            foreach (var groupType in _groupTypes) // Итерируем по всем определенным типам групп
            {
                ...
                {
                    MyLogger.LogWarning($"Путь для группы '{groupType}' не определен или пуст. Пропускаю.", MyLogger.LogCategory.Editor);
                    continue;
                }

                if (!Directory.Exists(folderPath))
                {
                    MyLogger.LogWarning($"Папка '{folderPath}' для группы '{groupType}' не существует. Пропускаю.", MyLogger.LogCategory.Editor);
                    continue;
                }
                ...
                if (targetGroup == null)
                {
                    ...
                    {
                        MyLogger.LogWarning($"Группа '{groupType}' не найдена. Попытка создать...", MyLogger.LogCategory.Editor);
                        ...
                        if (targetGroup == null)
                        {
                            MyLogger.LogError($"Не удалось создать группу '{groupType}' во время назначения ассетов. Пропускаю ассеты из '{folderPath}'.", MyLogger.LogCategory.Editor);
                            continue;
                        }
                        MyLogger.Log($"Создана группа '{groupType}' во время назначения ассетов.", MyLogger.LogCategory.Editor);
                    }
                    else
                    {
                        MyLogger.LogWarning($"Группа '{groupType}' не найдена и опция создания групп отключена. Использую группу по умолчанию.", MyLogger.LogCategory.Editor);
                        targetGroup = settings.DefaultGroup;
                        if (targetGroup == null)
                        {
                            MyLogger.LogError("Группа по умолчанию не найдена! Не могу обработать ассеты.", MyLogger.LogCategory.Editor);
                            return;
                        }
                    }
                }
            ...
            MyLogger.Log($"Обработка ассетов завершена: Просмотрено {assetsProcessedCount} ассетов. Добавлено/Обновлено {assetsAddedOrUpdatedCount} записей Addressables.", MyLogger.LogCategory.Editor);
        }
        ...
        private void ValidateAddressableSettings()
        {
            ...
            foreach (var filePath in scriptFiles)
            {
                ...
                catch (IOException ex) { MyLogger.LogWarning($"Не удалось прочитать файл {filePath}: {ex.Message}", MyLogger.LogCategory.Editor); }
            }
            ...
            else
            {
                ...
                foreach (var issue in issues) { resultMessage.AppendLine($"- ОШИБКА: {issue}"); MyLogger.LogError(issue, MyLogger.LogCategory.Editor); }

                resultMessage.AppendLine($"\nОбнаружено предупреждений: {warnings.Count}");
                foreach (var warning in warnings) { resultMessage.AppendLine($"- ПРЕДУПРЕЖДЕНИЕ: {warning}"); MyLogger.LogWarning(warning, MyLogger.LogCategory.Editor); }
            }

            EditorUtility.DisplayDialog("Результаты проверки Addressables", resultMessage.ToString(), "OK");
            MyLogger.Log("Проверка настроек Addressables завершена. Подробности выше и в консоли.", MyLogger.LogCategory.Editor);
        }
        #endregion
    }
}