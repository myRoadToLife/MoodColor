    ...
    public class FirebaseCacheManager
    {
        ...
        public FirebaseCacheManager(ISaveLoadService saveLoadService)
        {
            ...
            else
            {
                ...
                MyLogger.LogWarning("⚠️ Не удалось получить IDataRepository, будет использован InMemoryDataRepository", MyLogger.LogCategory.Firebase);
            }
            ...
            TimeSpan? expiration = null, bool forceRefresh = false)
        {
            ...
            {
                MyLogger.Log($"✅ Данные получены из памяти для ключа: {key}", MyLogger.LogCategory.Firebase);
            ...
            try
            {
                ...
                if (data != null)
                {
                    ...
                    MyLogger.Log($"✅ Новые данные загружены и кэшированы для ключа: {key}", MyLogger.LogCategory.Firebase);
                    return data;
                }
            }
            catch (Exception ex)
            {
                MyLogger.LogWarning($"⚠️ Ошибка при получении данных: {ex.Message}. Возвращаем кэшированные данные.", MyLogger.LogCategory.Firebase);
                ...
                {
                    MyLogger.Log($"✅ Данные восстановлены из диск-кэша для ключа: {key}", MyLogger.LogCategory.Firebase);
            ...
            {
                MyLogger.LogWarning($"⚠️ Используем устаревший кэш для ключа: {key}", MyLogger.LogCategory.Firebase);
                return JsonConvert.DeserializeObject<T>(expiredCache.Data);
            }

            MyLogger.LogError($"❌ Не удалось получить данные для ключа: {key}", MyLogger.LogCategory.Firebase);
            return default;
        }
        ...
        public void Set<T>(string key, T data, TimeSpan? expiration = null)
        {
            ...
            string json = JsonConvert.SerializeObject(data);
            CacheData(key, json, expiration ?? _defaultCacheExpiration);
            MyLogger.Log($"✅ Данные добавлены в кэш для ключа: {key}", MyLogger.LogCategory.Firebase);
        }
        ...
        public T Get<T>(string key, bool includeExpired = false)
        {
            ...
            {
                MyLogger.Log($"✅ Данные получены из кэша для ключа: {key}", MyLogger.LogCategory.Firebase);
                return JsonConvert.DeserializeObject<T>(cacheItem.Data);
            }
            ...
            {
                MyLogger.Log($"✅ Данные получены из диск-кэша для ключа: {key}", MyLogger.LogCategory.Firebase);
                // Обновляем кэш в памяти
                _memoryCache[key] = diskCache;
                return JsonConvert.DeserializeObject<T>(diskCache.Data);
            }

            MyLogger.LogWarning($"⚠️ Кэш не найден для ключа: {key}", MyLogger.LogCategory.Firebase);
            return default;
        }
        ...
        public void Remove(string key)
        {
            ...
            try
            {
                ...
                MyLogger.Log($"✅ Кэш удален для ключа: {key}", MyLogger.LogCategory.Firebase);
            }
            catch (Exception ex)
            {
                MyLogger.LogError($"❌ Ошибка при удалении кэша: {ex.Message}", MyLogger.LogCategory.Firebase);
            }
        }
        ...
        {
            _memoryCache.Clear();
            MyLogger.Log("✅ Кэш очищен", MyLogger.LogCategory.Firebase);
        ...
        private void CacheData(string key, string data, TimeSpan expiration)
        {
            ...
            {
                MyLogger.LogError($"❌ Ошибка при сохранении кэша: {ex.Message}", MyLogger.LogCategory.Firebase);
            }
        }
        ...
        private CacheItem LoadFromDiskCache(string key)
        {
            ...
            {
                MyLogger.LogError($"❌ Ошибка при загрузке кэша из хранилища: {ex.Message}", MyLogger.LogCategory.Firebase);
            }
        ...
        private void LoadCache()
        {
            ...
            MyLogger.Log("✅ FirebaseCacheManager инициализирован", MyLogger.LogCategory.Firebase);
        }
        #endregion
