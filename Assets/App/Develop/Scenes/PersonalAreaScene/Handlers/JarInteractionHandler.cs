...
namespace App.Develop.Scenes.PersonalAreaScene.Handlers 
{
    public class JarInteractionHandler : MonoBehaviour, IInjectable
    {
        ...
        {
            MyLogger.Log("[JarInteractionHandler] Inject method CALLED.");
            if (container == null)
            {
                MyLogger.LogError("[JarInteractionHandler] DIContainer is null in Inject method.");
                return;
            }
            ...
            {
                MyLogger.LogError("[JarInteractionHandler] IPointsService не удалось получить из DI контейнера!");
            }
            else
            {
                MyLogger.Log("[JarInteractionHandler] IPointsService успешно внедрен.");
            }

            _emotionService = container.Resolve<IEmotionService>();
            if (_emotionService == null)
            {
                MyLogger.LogError("[JarInteractionHandler] EmotionService не удалось получить из DI контейнера!");
            }
            else
            {
                MyLogger.Log("[JarInteractionHandler] EmotionService успешно внедрен.");
            }
        }
        ...
        public void OnJarClicked(string emotionTypeString)
        {
            if (_pointsService == null)
            {
                MyLogger.LogError("[JarInteractionHandler] _pointsService не инициализирован. Невозможно начислить очки. Убедитесь, что Inject был вызван.");
                return;
            }

            if (string.IsNullOrEmpty(emotionTypeString))
            {
                MyLogger.LogError("[JarInteractionHandler] Получена пустая строка типа эмоции.");
                return;
            }
            ...
            if (Enum.TryParse<EmotionTypes>(emotionTypeString, true, out EmotionTypes emotionType)) // true для case-insensitive
            {
                ...
                MyLogger.Log($"[JarInteractionHandler] Клик по банке {emotionType}. Начислено {POINTS_PER_JAR_CLICK} очков. Источник: {PointsSource.JarInteraction}, Описание: {detailedDescription}. Текущие очки: {_pointsService.CurrentPoints}");
                ...
                {
                    MyLogger.LogError("[JarInteractionHandler] EmotionService is not injected!");
                }
            ...
            {
                MyLogger.LogError($"[JarInteractionHandler] Не удалось распознать тип эмоции из строки: {emotionTypeString}");
            }
        }
    }
} 